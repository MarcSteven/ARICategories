...
@interface TapRichText : NSObject
...
- (void (^)(NSString *, NSRange, NSInteger))tapActionBlock {
    return objc_getAssociatedObject(self, _cmd);
}

- (void)setTapActionBlock:(void (^)(NSString *, NSRange, NSInteger))tapActionBlock {
...
- (void)po_tapRangeActionWithStringArray:(NSArray<NSString *> *)stringArray tapActionBlock:(void (^)(NSString * _Nonnull, NSRange, NSInteger))tapActionBlock {
    //获取部分点击的区域
    [self tagRangeActionWithString:stringArray];
    
    if (self.tapActionBlock != tapActionBlock) {
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event {
    ...
    [self tapFrameWithTouchPoint:point result:^(NSString *string, NSRange range, NSInteger index) {
        ...
        if (weakSelf.tapActionBlock) {
            weakSelf.tapActionBlock (string , range , index);
        }
    ...
    for (CFIndex i = 0; i < count; ++i) {
        ...
        if (CGRectContainsPoint(rect, point)) {
            ...
            for (int j = 0; j < link_count; ++j) {
                ...
                TapRichText *model = self.attributeStrings[j];
...
#pragma mark - 是否显示点击效果  默认是   项目中一般是实现点击效果
...
- (void)tapEffectWithStatus:(BOOL)status {
    if (self.isTapEffect) {
        ...
        if (status) {
            ...
            [subAtt addAttribute:NSForegroundColorAttributeName value:[self.textColor colorWithAlphaComponent:0.5] range:NSMakeRange(0, subAtt.string.length)];
...
- (void)tagRangeActionWithString:(NSArray<NSString *> *)strings {
    ...
    [strings enumerateObjectsUsingBlock:^(NSString * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if (range.length != 0) {
            ...
            TapRichText *model = [TapRichText new];
            model.range = range;
            model.string = obj;
