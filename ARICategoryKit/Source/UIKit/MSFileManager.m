...
+ (NSArray *)listFilesInHomeDirectoryByDeep:(BOOL)deep {
    return [self listFilesInDirectoryAtPath:[MSFileManager homePath] deep:deep];
}

+ (NSArray *)listFilesInLibraryDirectoryByDeep:(BOOL)deep {
    return [self listFilesInDirectoryAtPath:[MSFileManager libraryPath] deep:deep];
}

+ (NSArray *)listFilesInDocumentDirectoryByDeep:(BOOL)deep {
    return [self listFilesInDirectoryAtPath:[MSFileManager documentsPath] deep:deep];
}

+ (NSArray *)listFilesInTmpDirectoryByDeep:(BOOL)deep {
    return [self listFilesInDirectoryAtPath:[MSFileManager tmpPath] deep:deep];
}

+ (NSArray *)listFilesInCachesDirectoryByDeep:(BOOL)deep {
    return [self listFilesInDirectoryAtPath:[MSFileManager cachesPath] deep:deep];
}
...
+ (BOOL)clearCachesDirectory {
    ...
    for (NSString *file in subFiles) {
        NSString *absolutePath = [[MSFileManager cachesPath] stringByAppendingPathComponent:file];
...
+ (BOOL)clearTmpDirectory {
    ...
    for (NSString *file in subFiles) {
        NSString *absolutePath = [[MSFileManager tmpPath] stringByAppendingPathComponent:file];
        isSuccess &= [self removeItemAtPath:absolutePath];
    }
    return isSuccess;
}
