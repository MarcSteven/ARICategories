...
public extension UITableView {
     static func build(block:@escaping ((UITableView)->Void)) ->UITableView {
...
public extension UITableView {
    ...
    ///                     for the row when scrolling concludes. The default value is `UITableViewScrollPositionNone`.
     func selectRows(at indexPaths: [IndexPath], animated: Bool, scrollPosition: ScrollPosition = .none) {
        indexPaths.forEach {
...
public extension UITableView {
     func dequeueReusableCell(ofClass cls:TableViewCellIdentifier.Type,
...
public extension UITableView {
    ...
    /// }
    ///```
     func snapRowsToTop(_ targetContentOffset: UnsafeMutablePointer<CGPoint>, cellHeight: CGFloat, headerHeight: CGFloat) {
        // Adjust target offset so that cells are snapped to top
